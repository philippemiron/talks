...
\newline\newline
RAFOS experiments sponsored by the Bureau of Ocean Energy Management (July 2011 - May 2015) courtesy Alexis Lugo Fernandez:
\begin{itemize}
...
Example, if $F$ moves 2 units to the right the distribution $f(x)$.
\begin{figure}[!ht]
\centering
\begin{tikzpicture}
% plot curve at t0
...
In our case, \underline{$F$ is unknown and we only have access to trajectories}. We will use a discrete method to approximate $\mathcal{P}$.
}
...
\begin{figure}
  \includegraphics[width=8.5cm]{geogomdeep-fig09.jpg}
\end{figure}
